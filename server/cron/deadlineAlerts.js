...
cron.schedule('*/5 * * * *', async () => {
  try {
    ...
    for (const lead of leads24h) {
      await createNotification({ message: `Urgent: Only 24 hours remaining for ${lead.owner || 'Unassigned'} to contact lead ${lead.company} (ID: ${lead.leadId || lead._id})`, type: 'urgent', category: 'Leads', recipientUser: lead.owner, relatedId: lead._id });
    ...
    for (const lead of leads1h) {
      await createNotification({ message: `Critical: Last 60 mins left for ${lead.owner || 'Unassigned'} to contact lead ${lead.company} (ID: ${lead.leadId || lead._id})`, type: 'urgent', category: 'Leads', recipientUser: lead.owner, relatedId: lead._id });
    ...
    for (const deal of activeDeals) {
      ...
      if (timeDiff > 0 && timeDiff <= 7 * 24 * 60 * 60 * 1000 && !deal.alert7dSent) {
        if (deal.owner) {
          await createNotification({
            message: `Deal Close Date Warning: The close date for deal ${deal.title} (ID: ${deal.dealId || deal._id}) owned by ${deal.owner || 'Unassigned'} is approaching in less than 7 days (${deal.close_date}).`,
            type: 'info',
            category: 'Deals',
      ...
      if (timeDiff > 0 && timeDiff <= 24 * 60 * 60 * 1000 && !deal.alert24hSent) {
        if (deal.owner) {
          await createNotification({
            message: `Urgent Close Warning: The close date for deal ${deal.title} (ID: ${deal.dealId || deal._id}) owned by ${deal.owner || 'Unassigned'} is in less than 24 hours!`,
            type: 'urgent',
            category: 'Deals',
            recipientUser: deal.owner,
            relatedId: deal._id
  ...
  try {
    ...
    if (overdueLeads.length > 0) {
      ...
      for (const [owner, count] of Object.entries(overdueByOwner)) {
        await createNotification({
          message: `Action Required: ${owner} has ${count} overdue leads (alert generated by System).`,
          type: 'warning',
          category: 'Leads',
    ...
    if (overdueDeals.length > 0) {
      ...
      for (const [owner, count] of Object.entries(overdueDealsByOwner)) {
        await createNotification({
          message: `Action Required: ${owner} has ${count} overdue Deals past close date (alert generated by System).`,
          type: 'warning',
          category: 'Deals',
          recipientRoles: ['Super Admin', 'Admin']
        });
