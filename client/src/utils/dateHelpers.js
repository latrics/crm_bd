...
export function getQuickRange(key) {
  ...
  if (key==='this_month') return { start:toLocalISO(new Date(y,m,1)), end:toLocalISO(new Date(y,m+1,0)) };
  if (key==='last_month') return { start:toLocalISO(new Date(y,m-1,1)), end:toLocalISO(new Date(y,m,0)) };
  if (key==='this_quarter') { const q=Math.floor(m/3); return { start:toLocalISO(new Date(y,q*3,1)), end:toLocalISO(new Date(y,q*3+3,0)) }; }
